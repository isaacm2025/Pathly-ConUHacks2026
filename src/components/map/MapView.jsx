...
export default function MapView({
  ...
  const getActivityStyle = (people) => {
    ...
    // Low activity (0): Medium blue #3B82F6 (59, 130, 246)
    // High activity (1): Darker red #DC2626 (220, 38, 38)

    // Interpolate between blue and red based on activity level
    const r = Math.round(59 + (220 - 59) * clamped);   // 59 -> 220
    const g = Math.round(130 + (38 - 130) * clamped);  // 130 -> 38
    const b = Math.round(246 + (38 - 246) * clamped);  // 246 -> 38

    const color = `rgb(${r}, ${g}, ${b})`;

    // Thicker lines for busier streets
    const weight = 2 + clamped * 2;

    // Higher opacity for busier streets
